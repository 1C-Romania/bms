...
Function ExternalDataProcessorInfo() Export
	...
	RegistrationParametrs.Insert("Description", "Forma de listare Aviz de expeditie(Vanzare bunuri)");
	...
	RegistrationParametrs.Insert("Information", "Forma de listare Aviz de expeditie(Vanzare bunuri)");
	
	CommandTable = GetCommandTable();
	
	AddCommand(CommandTable,
	"Aviz de expeditie",						    // what we will see under button PRINT
	"AvizDeExpeditie",   						// Name of Template 
...
Function CreatePrintForm(ObjectArray, PrintObjects, TemplateName)	
	...
	|	InventoryExpense.NoticeSeries AS SerieAviz,
	|	InventoryExpense.NoticeNumber AS NumarAviz
	|FROM
	...
	While Selection.Next() Do
	    ...
	    If TemplateName <> "AvizDeExpeditie" Then 
        ...
        If TemplateName <> "AvizDeExpeditie" And ValueIsFilled(Selection.GoodsOrder)   Then 
			Header.Parameters["ShippingDate"] =  Format(Selection.GoodsOrder.ShippingDate,"DF=""dd.MM.yyyy""");
		EndIf;  
		     If TemplateName <> "AvizDeExpeditie" And ValueIsFilled(Selection.Responsible)   Then 
...
/////de pus Selection//////Header.Parameters ["EmailAgent"]   =    Selection.Author;
		...
		Header.Parameters["NumarAviz"] =  Selection.NumarAviz;
		Header.Parameters["SerieAviz"] =  Selection.SerieAviz;
...
VATAmount = Format(Round((Amount	* SelectionInventory.VATRate.Rate)/100,2),	"NFD=2");
		...
		AreaInventory.Parameters["Amount"]    = Amount-(Reducere*SelectionInventory.Quantity);
			...
			//// ///Adelin Serb 10.02.2015 If
		If TemplateName =  "AvizDeExpeditie" Then
			AreaInventory.Parameters["VATAmount"] = SelectionInventory.VATAmount;
			EndIf;
			///Adelin Serb 10.02.2015 EndIf
			...
			TotalVAT 	= TotalVAT		+ VATAmount-(VATAmount/100*RataReducere);
		...
		Try
			Footer.Parameters["TotalTVA"]			= Format(Selection.DocumentVATAmount,				"NFD=2");
			Footer.Parameters["TotalCuTVA"]			= Format((Selection.DAmountWithoutVAT + Selection.DocumentVATAmount),"NFD=2");
		Except
		
		EndTry;
		
		Footer.Parameters["TotalFaraTVA"]		= Format(Selection.DAmountWithoutVAT,			"NFD=2");
		...
		If TemplateName  <>  "AvizDeExpeditie" And  CommaPosition > 0 Then
