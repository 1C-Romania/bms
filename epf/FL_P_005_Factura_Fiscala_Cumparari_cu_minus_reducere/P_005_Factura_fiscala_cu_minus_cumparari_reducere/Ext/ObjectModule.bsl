...
Function ExternalDataProcessorInfo() Export
	...
	RegistrationParametrs.Insert("Description", "Formele de listare Factura fiscala(minus)");
	RegistrationParametrs.Insert("Version", "1.0"); 
	RegistrationParametrs.Insert("SafeMode", False);
	RegistrationParametrs.Insert("Information", "Formele de listare Factura fiscala(minus)");
...
If SelectionInventory.DiscountMarkupRate >0 Then
	...
	SUMA=Format(Round("-"+(SelectionInventory.Price *SelectionInventory.Quantity)*ERate,2),"NFD=2");

	AreaInventory.Parameters["Amount"]= SUMA;  		
	
	Try

	AreaInventory.Parameters["VATAmount"]	=Format(Round((SUMA * SelectionInventory.VATRate)/100,2),"NFD=2");;
Except
...
If SelectionInventory.DiscountMarkupRate >0 Then
	     ...
	     SUMATVA= Format(Round((SUMA * SelectionInventory.VATRate)/100,2),"NFD=2");;

				
		 AmountDiscount=(SUMA* SelectionInventory.DiscountMarkupRate)/100;

		  AreaInventory1.Parameters["AmountDiscount"] = "+" + Format(AmountDiscount * -1,"NFD=2");
		  
		  TVADiscount = (SUMATVA* SelectionInventory.DiscountMarkupRate)/100;
		  
		AreaInventory1.Parameters["VATAmountDiscount"] = "+"  + Format(TVADiscount* -1,"NFD=2");
	...
	EndIf;
		...
		AdosSuma=Format((SUMA* SelectionInventory.DiscountMarkupRate)/100,"NFD=2");
		
		AreaInventory1.Parameters["AmountDiscount"]= "-" + AdosSuma;
		
		AdaosTVA=Format((SUMATVA* SelectionInventory.DiscountMarkupRate)/100,"NFD=2");
		AreaInventory1.Parameters["VATAmountDiscount"]= "-" + AdaosTVA; 
		EndIf;
